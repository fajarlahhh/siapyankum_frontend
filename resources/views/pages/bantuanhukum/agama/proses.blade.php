...
@section('header')
	<h1 class="page-header">Bantuan Hukum / Agama <small>{{ $aksi }} Data</small></h1>
@endsection
...
@section('subcontent')
	<div class="panel panel-inverse" data-sortable-id="form-stuff-1">
		...
		<form action="{{ route('agama.'.strtolower($aksi)) }}" method="post" data-parsley-validate="true" data-parsley-errors-messages-disabled="">
			@csrf
...
@push('scripts')
	...
	<script>
		...
		function hapus(id, status, nama, tanggal) {
			swal({
				title: 'Hapus Data',
				text: 'Anda akan menghapus proses bantuan hukum agama : ' + nama + ' dengan status : ' + status,
				icon: 'warning',
				buttons: {
					cancel: {
						text: 'Batal',
		      	...
		      	if (isConfirm) {
	          		...
	          		$.ajax({
	          			url: "/agama/hapusproses/" + id + "/" + status + "/" + tanggal,
	          			type: "POST",
	          			data: {
	          				"_method": 'DELETE',
	          			},
